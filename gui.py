...
def encryptInputText(InputBox, OutputBox):
    ...
    if choice == 0:
        chiffreText =ceasarChiffre(content, 12)

    if choice == 1:

        print('In chocie 2')
        chiffreText = vigenereChiffre(content, "DefaultKey")


    print(f"Choice: {choice} , text: {chiffreText}")
    OutputBox.delete("1.0", "end")
    OutputBox.insert("1.0", chiffreText)
...
newFrame.grid(column = 2, row = 3)

v = tk.StringVar(newFrame, 0)

r1 = tk.Radiobutton(newFrame, text = "Caesar-Chiffre", value = 0, variable = v)
r1.pack()

r2 = tk.Radiobutton(newFrame, text = "Vigenere-Chiffre", value = 1, variable = v)
r2.pack()
...
Button = tk.Button(text = "Save to file ", command = lambda: openFile(RightBox))
